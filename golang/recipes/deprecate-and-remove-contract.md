...
- HTTP: `go test ./internal/api/http/... -run TestDeprecatedRouteHeaders` asserts `Deprecation`/`Sunset` headers during the window, and the route's absence (`404`/`410`) after removal — see [../reference/exampleservice/](../reference/exampleservice/)
