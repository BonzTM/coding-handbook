...
Ownership-transfer checklist for a Go repo built from this handbook. Walk it with both the outgoing and incoming owner present. See [../onboarding-and-handoff.md](../onboarding-and-handoff.md) for the day-one reading path and the questions the new owner must be able to answer.
...
- [ ] `.env.example` confirmed in sync with the current `internal/config` loader.
...
- [ ] Dependency-update ownership transferred, including who runs `make vuln` and acts on findings.
...
- [ ] New owner ran `make verify` from a clean clone and it passed on their machine.
