<!--
Template: downstream CONTRIBUTING.md for a repo built on the Go handbook.
...
Thanks for contributing. This repo follows the [Go Project Handbook](<HANDBOOK_URL>); this file is the local entry point and points at the handbook for the detail. When in doubt, the handbook is the contract.
...
`make verify` is the single gate: it runs tidy, fmt-check, lint, `go vet`, tests, `-race`, `govulncheck`, and build. It must be green before you open a PR. Coverage is a separate `make cover`. Requires Go <GO_VERSION> or newer; tool dependencies are managed via `go get -tool` / `go tool` (no `tools.go`).
...
- A PR merges only after CI `make verify` is green and at least one non-author approval. Fill in the PR template at <PR_TEMPLATE_PATH>.
...
- Contracts, config keys, and exported symbols are documented; backward-incompatible changes carry a deprecation plan.
- `make verify` is green from a clean tree; coverage did not regress on mandatory paths.
- Operator-visible changes (new env vars, migrations, ports, contract changes) have a `CHANGELOG.md` entry — see <CHANGELOG_PATH>.
