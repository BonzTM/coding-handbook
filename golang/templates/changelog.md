<!--
Template: CHANGELOG.md for a repo built on the Go handbook.
...
- VCS tags are canonical Go module versions: v<MAJOR>.<MINOR>.<PATCH> (the "v"
  prefix is required). The headings below drop the "v" per Keep a Changelog;
  the git tag must keep it (tag v1.2.3 <-> heading [1.2.3]).
- Every operator-visible change (env vars, migrations, ports, contract changes)
  MUST have an entry here. See <HANDBOOK_URL>/operations/ci-and-release.md.
-->
