...
// boundary (authorize at the edge, not in helpers).
type Role string
...
// It is a plain value type with no wire concerns: the auth interceptor builds it
// from validated credentials and threads it through the request context.
// Subject and TenantID are required; Roles may be empty (an authenticated but
// unprivileged caller).
type Principal struct {
	// Subject is the token "sub": the stable caller identity.
	Subject string
	// TenantID is the tenant the caller acts within. Every store query is scoped
	// by it so one tenant can never read another's rows.
	TenantID string
	// Roles are the caller's authorization roles.
	Roles []Role
}
...
// The auth interceptor calls it after verifying a token; the service reads it
