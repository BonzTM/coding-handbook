...
var (
	...
	// It is mapped to a 400 at the transport boundary.
	ErrInvalidCursor = errors.New("invalid cursor")
)

// Pagination bounds for List endpoints, per
// golang/services/http-services.md ### List Endpoints And Pagination: the
// server enforces a default and a maximum page size and clamps an oversized
...
// cursor encodes to the empty string so a last page reports next_cursor: "".
...
// so the transport can map it to a 400 instead of a 500.
...
// the empty string in the wire envelope.
type Page struct {
	Widgets    []Widget
	NextCursor Cursor
}
...
//
// Every method is scoped by tenantID: the store filters on the tenant_id column
// so one tenant can never observe another's rows. Multi-tenancy is enforced in
// the storage layer (the WHERE clause), not only at the edge, so a missing
// boundary check cannot leak cross-tenant data. See golang/services/database.md.
type Store interface {
	...
	// derives the next cursor from the last returned widget. Implementations MUST
	// honor the same total ordering so pages neither overlap nor skip rows.
...
// boundary authorization check lives here, not in a helper, per
// golang/services/http-services.md. The widget's TenantID is taken from the
// principal, never from the request body, so a caller cannot write into another
// tenant. Validation failures return ErrInvalidWidget; a duplicate ID surfaces
// the store's ErrAlreadyExists.
...
// clamped down to it rather than rejected, per the handbook's List Endpoints
