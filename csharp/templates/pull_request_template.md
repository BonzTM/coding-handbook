...
- [ ] Docs / tooling only
- [ ] Dependency bump
- [ ] Breaking change (public API, wire, schema, config, or event contract)

## Gates

- [ ] `pwsh ./verify.ps1` is green locally (restore (locked), format-check, build (warnings-as-errors), test, audit).
...
- [ ] Docs and the README config table updated for any new or changed config key.
...
- [ ] Compatibility / migration impact noted below if public API, wire, schema, or event payloads changed.
