...
- A **major dependency or framework exception** — any escalation past a [framework-selection.md](framework-selection.md) default: Dapper replacing EF Core on a measured hot path, MassTransit over the repo-owned messaging seam, a Blazor frontend instead of Razor Pages, a third-party DI container over `Microsoft.Extensions.DependencyInjection`.
- A **publish-model change** — enabling Native AOT or ReadyToRun, which constrains reflection, serialization, and dependency choices for every later change.
...
- Any **deviation from a handbook default** (solution layout, error contract, logging, config posture, dependency posture).

If the decision is obvious, trivially reversible, and local to one project, skip the ADR — the code and its tests are the record. When in doubt, the test is: would a competent new owner be surprised by this, and is undoing it expensive? If yes, write the ADR.
...
decisions/0003-dapper-for-order-lookup-hot-path.md
```
...
- The title line and the filename agree: `# 0003. dapper for order lookup hot path`.
