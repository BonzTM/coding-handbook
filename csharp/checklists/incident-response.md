...
- [ ] If load-driven, scale out within the runbook's safe bounds (mind database connection-pool limits before adding replicas).
...
- [ ] Read metrics: which SLI broke, error-class breakdown, latency tail, saturation of the suspected resource (thread-pool starvation, connection-pool exhaustion, GC pressure).
- [ ] Read logs filtered to the incident window and the affected component; pivot on the trace/request ID rather than guessing.
